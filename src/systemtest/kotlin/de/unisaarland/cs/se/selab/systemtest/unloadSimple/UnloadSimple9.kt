 ...
 */
class UnloadSimple9 : SystemTestExtension() {
    // short description of the test
    override val description = "Simple Unloading Test till tick 9"

    // name of the test
    override val name = "Unload Simple 9"
    ...
    private suspend fun manageCorpActions(tick: Int) {
        ...
        if (tick == 7) {
            assertNextLine("Current Drift: Ship 2 drifted from tile 66 to tile 67.")
        } else if (tick == 8) {
            assertNextLine("Current Drift: Ship 2 drifted from tile 65 to tile 66.")
        } else if (tick == 9) {
            assertNextLine("Current Drift: Ship 2 drifted from tile 64 to tile 65.")
        }
    }
    ...
    private suspend fun moveShips2(tick: Int) {
        when (tick) {
            6 -> {
                ...
                assertNextLine("Ship Movement: Ship 2 moved with speed 10 to tile 58.")
            ...
            7 -> {
                assertNextLine("Ship Movement: Ship 1 moved with speed 10 to tile 42.")
                assertNextLine("Ship Movement: Ship 2 moved with speed 20 to tile 66.")
                assertNextLine("Ship Movement: Ship 4 moved with speed 75 to tile 74.")
            }
            8 -> {
                assertNextLine("Ship Movement: Ship 1 moved with speed 20 to tile 22.")
                assertNextLine("Ship Movement: Ship 2 moved with speed 20 to tile 65.")
            }
            9 -> {
                assertNextLine("Ship Movement: Ship 0 moved with speed 25 to tile 74.")
                assertNextLine("Ship Movement: Ship 1 moved with speed 30 to tile 15.")
                assertNextLine("Ship Movement: Ship 2 moved with speed 20 to tile 64.")
